...
def fig_micrometers():
    ...
    """
    answer = input('Which objective size have you used? Options are 4x, 10x and 20x.')
...
def open_layout():
    ...
    df = pd.read_csv(date + '/layout_' + date + '.csv', sep = ',')
    df.head(20)
    return df, date
...
def open_df(well, date, pic_total_area, terminator = "_data", area_column = "Area (pixel^2)"):
    """
    Open dataframe, create cell-covered column, and return df.
    terminator = "_data" is standard, change if different.
    area_column = "Area (pixel^2) standard. Provide the column that has the scratch measured area in pixels. 
    """
    df = pd.read_csv(date + '/' + well + terminator + ".csv", sep="\t|:|;", encoding = "unicode_escape", engine = "python")
...
def plot(df, layout, begin, end, index, date, well, time, pic_total_area, x_fit, y_fit, time_column = "Time (min)"):
    """
    ...
    plt.savefig(date + '/layout_' + well + '.png', dpi = 150, bbox_inches='tight')
    plt.show()
    plt.cla()
    plt.clf()
    plt.close('all')
...
def plot_all(layout, hueorder, date, xaxis = "Cell", yaxis = "Velocity", ylimit = (0,1), ylabel = "Cell migration velocity \u03bcm/min"):
    """
    ...
    plt.show()
    g.savefig(date + "/" + yaxis + date + '.tiff')
    g.savefig(date + "/" + yaxis + date + '.svg')
    sns.reset_orig()
...
def import_results():
    ...
    """
    date = input('Which is the date of your experiment (YYYYMMD)? Provide the path if different from YYYYMMDD')
    df = pd.read_csv(date + '/Results_' + date + '.csv', sep = ',')
    df.head(20)
    return df, date
